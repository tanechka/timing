...
import ComplexityRange from 'app/components/calculator/ComplexityRange'
import format from 'app/services/format'

export default class Header extends React.PureComponent {
  render () {
    const {setComplexity, complexity, result} = this.props
      ...
      <header className='calculator-header'>
        ...
        <div className='calculator-header-content pull-right'>
          ...
          <div className='calculator-header-item header-time pull-left'>
            ...
            <span className='range'>{format.hours(result.get('hoursFrom'))}
              - {format.hours(result.get('hoursTo'))}</span>
          </div>
